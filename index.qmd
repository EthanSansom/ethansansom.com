...
mouseenter = function(d) {
  // We're mousing over the parent <g> container, which is `this` but we really 
  ...
  for (var i = 0; i < summaries.length; i++) {
    ...
    const current_group = parent; // d3.select(parent.nodes()[i]) // <g> element we're appending to
...
mouseleave = function(d) {
  // Remove any point text or titles from the grid
  svg
    .selectAll(".point-title")
    .remove();
  svg
