...
href_posts_root <- "http://localhost:5788/"
...
posts_metadata$idx <- sample(1:3, size = nrow(posts_metadata), replace = TRUE)
...
// Make a container for the projects points to live
project_points = svg.append("g").classed("project-points", true);
...
// TODO: Read more about the D3 enter, exit, update idea. `enter()` seems good.
enter = function(data) {
  // Add the grid points
...
// TODO: This doesn't work. Think more about how to do the sequenced enter, exit
// and update. Here's an article that might help, although it's not exactly my
// use case.
//
// https://observablehq.com/@thetylerwolf/day-18-join-enter-update-exit
//
// MAKE a simple example, without trying to fill in all the text. Just make
// the grid boxes move around to the new location and spawn in the new data /
// remove the old data within a single .join()
//
/*
  .join(
    enter => enter
      ...
      .on("mouseleave", mouseleave)
      .etc...,
    // NOTE: We can simply delay the enter or exit accordingly so we can sync
    // them being one after another.
    exit => exit
      .dont_allow_interaction()
      // NOTE: We'll need to pass in the new data somehow to the old data
      // in order to move the old data to a new location before fading away.
      .move_to_new_location_or_fade(new_data)
      .remove()
    )
*/
update = function(data) {
  ...
  // If for any reason a grid-point is still "open", de-expand it back to it's
  ...
  grid_points
    ...
    .call(
      ...
      points => points
        ...
        .duration("200")
        .style("opacity", 0)
        // HACK: To make sure the exit and entrance of the new data happens after
        // this transition, I'm calling enter and exit here.
        .on("end", function() {
          exit(data);
          enter(data);
        })
