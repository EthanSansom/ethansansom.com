...
mouseover = function(d) {
  ...
  // Determine where the current point and textbox live
  ...
  const expanded_width = maximum_chars_per_line * 0.95;
  const expanded_height = (n_lines * line_height) + (point_title_size * 4) + point_text_size;
  d3.select(this)
    .transition()
    .duration("300")
    .attr("x", function() { 
      if (d3.select(this).attr("data-expand-dir-x") === "right") {
        return d3.select(this).attr("data-original-x")
      }
      else {
        return d3.select(this).attr("data-original-x") - expanded_width + point_width;
      }
    })
    .attr("y", function() { 
      if (d3.select(this).attr("data-expand-dir-y") === "down") {
        return d3.select(this).attr("data-original-y")
      }
      else {
        return d3.select(this).attr("data-original-y") - expanded_height + point_height; 
      }
    })
    .attr("width", expanded_width)
    .attr("height", expanded_height)
    .attr("fill-opacity", 1)
    
  // TODO: 
  // - We still need to place the text according to where the point is going to expand
  // - We also need to set the mouseover and mouseleave to the parent element <g> rather
  //   than on the <rect> element, which means here we will manipulate `this.select()` rather
  //   than the `parent`.
  
  // We've already added the text, so now we just fade it in
...
mouseleave = function(d) {
  // Fade out and then remove any text generated during mouse over
  d3.selectAll(".point-title")
    .transition()
    .duration("100")
    .style("opacity", 0)
    .on("end", function() {
