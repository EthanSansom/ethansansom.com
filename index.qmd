...
# Attempts to parse only up to line `n` (default `n = 50` should be sufficient).
read_post_yaml <- function(path, n = 50) {
  lines <- readLines(path, n = n)
  yaml_breaks <- which(lines == "---")
  if (length(yaml_breaks) < 2) {
    stop(paste0("File '", path, "' does not contain a YAML fence."))
  }
  out <- data.frame(
    title = "",
    date = "",
